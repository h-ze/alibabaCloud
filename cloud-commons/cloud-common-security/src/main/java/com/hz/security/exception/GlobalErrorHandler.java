package com.hz.security.exception;
...
public class GlobalErrorHandler {
    ...
    public Object exceptionHandler(Exception ex) {
        ...
        if(ex instanceof ConstraintViolationException ||ex instanceof IllegalArgumentException){
            ...
            return ResponseResult.errorResult(HttpStatus.INTERNAL_SERVER_ERROR.value(), ex.getMessage());
        }

        if(ex instanceof BlockException){
            /*log.error("Sentinel阻塞异常{}", printErrorStack(ex));*/
            log.error(printErrorStack(ex));
            return ResponseResult.errorResult(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Sentinel Blocked The Request");
        }
        if(ex instanceof SentinelRpcException){
            /*log.error("Sentinel调用异常{}", printErrorStack(ex));*/
            log.error(printErrorStack(ex));
            return ResponseResult.errorResult(HttpStatus.INTERNAL_SERVER_ERROR.value(), "An Exception Occurred While Sentinel Execution Rpc Request");
        }
        if(ex instanceof NoHandlerFoundException){
            /*log.error("未找到资源异常{}", printErrorStack(ex));*/
            log.error(printErrorStack(ex));
            return ResponseResult.errorResult(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.getReasonPhrase());
        }

        if (ex instanceof PermissionException){
            log.error(printErrorStack(ex));
            return ResponseResult.errorResult(200000, ex.getMessage());
        }
        /*log.error("系统服务异常{}", printErrorStack(ex));*/
        log.error(printErrorStack(ex));
        return ResponseResult.errorResult(HttpStatus.INTERNAL_SERVER_ERROR.value(), HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase());
    }
