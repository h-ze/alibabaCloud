...
public class AuthorizeFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        MDC.put(GatewayCoreConstant.GLOBAL_LOG_PRIFIX, globalLogPrifix);
        String token=exchange.getRequest().getHeaders().getFirst(GatewayCoreConstant.X_AMZ_SECURITY_TOKEN);
        if(StringUtils.isEmpty(token)){
            addHeader(exchange.getRequest().mutate(), GatewayCoreConstant.GLOBAL_LOG_PRIFIX,globalLogPrifix);
        ...
        if(StringUtils.isNotEmpty(userStr)){
            addHeader(exchange.getRequest().mutate(),GatewayCoreConstant.USER_PRINCIPAL,userStr);
        }
